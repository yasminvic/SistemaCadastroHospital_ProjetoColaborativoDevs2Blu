...
namespace Devs2Blu.ProjetosAula.OOP3.Main.Utils
{
    public class Mocks
    {
        ...
        {
            Random rd = new Random();
            String[] setores = { "Atendimentos", "Quartos", "Saquão", "Agendamentos" };
            for (int i = 0; i < 4; i++)
            {
                Recepcionista recepcionista = new Recepcionista(i, $"Recepcionista {i + 1}", $"{i + rd.Next(0, 5)}23{i + rd.Next(0, 5)}56{i + rd.Next(0, 5)}891{i + rd.Next(0, 5)}", setores[rd.Next(0, 3)]);
